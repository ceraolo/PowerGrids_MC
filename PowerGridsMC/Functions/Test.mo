...
// When simulation ends with time=0, we have reached the synchronous speed
    ...
    annotation(
      experiment(StopTime = 0.976, StartTime = 0, Tolerance = 1e-06, Interval = 0.001952),
...
// da togliere successivamente:
      ...
      Icon(coordinateSystem(preserveAspectRatio = false), graphics = {Text(extent = {{-100, 140}, {102, 110}}, lineColor = {85, 0, 255}, fillColor = {95, 95, 95}, fillPattern = FillPattern.Solid, textString = "%name")}),
...
<p>This SS operating point is useful for the implementation of PFT behaviour when asynchronous machines are incudes.</p>
  ...
  model TestStartAndSS_MSL "Compares MSL and PowerGridsMC asynchronous machine models start from standstill"
    ...
    Modelica.Mechanics.Rotational.Components.Inertia inertiaPg(J = 2, phi(fixed = true, start = 0)) annotation(
    ...
    Modelica.Mechanics.Rotational.Sources.ConstantTorque tau1(tau_constant = -T1) annotation(
      Placement(transformation(origin = {-6, 44}, extent = {{80, -54}, {60, -34}})));
    Functions.Test.FindSteadyState findSS(V = V, T1 = T1, T2 = T2) annotation(
      Placement(transformation(extent = {{-12, 2}, {8, 22}})));
  equation
    connect(pMecc.flange_b, inertiaPg.flange_a) annotation(
      Line(points = {{10, -26}, {16, -26}}, color = {0, 0, 0}));
    connect(bus.terminal, aimcPg.terminal) annotation(
      Line(points = {{-62, -26}, {-46, -26}}));
    connect(aimcPg.flange, pMecc.flange_a) annotation(
      Line(points = {{-26, -26}, {-10, -26}}));
    connect(tau1.flange, inertiaPg.flange_b) annotation(
      Line(points = {{54, 0}, {48, 0}, {48, -26}, {36, -26}}));
    connect(tau2.flange, inertiaPg.flange_b) annotation(
      Line(points = {{54, -44}, {48, -44}, {48, -26}, {36, -26}}));
    annotation(
      Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-80, -60}, {80, 40}})),

...
// The simplified model has no saturation, to make the controller equations linear
    y = homotopy(actual = smooth(0, if u < uMin then u elseif u > uMax then u else 0), simplified = u);
  else
// The simplified model has no saturation, to make the controller equations linear
    y = homotopy(actual = if u < uMin then u elseif u > uMax then u else 0, simplified = u);
  end if;
  annotation(

...
model RampTrackingFilter "Ramp tracking filter"
  ...
  Dialog(group="Initialization"));
  PowerGridsMC.Controls.LeadMOrderLag leadMOrderLagCascade[N](each M = M, each T1 = T1, each T2 = T2, each initType = initType, each k = 1, each yStart = yStart)  annotation (

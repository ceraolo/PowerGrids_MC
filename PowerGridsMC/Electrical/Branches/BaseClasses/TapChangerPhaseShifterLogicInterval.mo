within PowerGridsMC.Electrical.Branches.BaseClasses;
...
protected
    ...
    Boolean valueInRange = not
                              (pre(valueUnderMin)) and not
                                                          (pre(valueAboveMax)) "true if the monitored signal is into the [valueMin ; valueMax] range";
...
equation
  ...
  when pre(running) and not
                           (pre(locked)) and pre(valueUnderMin) then
  ...
  when not
          (pre(running)) or pre(locked) then
    state = State.locked;
    tap = pre(tap);
    tTapMoved = timeInfinity;

  //Transition from "locked" to "standard"
  elsewhen pre(running) and pre(state) == State.locked and not
                                                              (pre(locked))
                   and valueInRange

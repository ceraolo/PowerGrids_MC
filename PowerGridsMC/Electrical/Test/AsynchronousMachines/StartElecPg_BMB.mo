...
model StartElecPg_BMB "Compares MSL and PowerGridsMC asynchronous machine models start from standstill"
  ...
  Electrical.Machines.AsynchronousMachine aimcPg(startSteadyState = true,
  J = aimcData.Jr,
...
//  UNom = 100,
  wMechanical(start = 0, fixed = true)) annotation(
...
equation
  ...
  connect(bus.terminal, aimcPg.terminal) annotation(
    Line(points = {{-36, -44}, {-20, -44}}));
  connect(aimcPg.flange, pMeccQs.flange_a) annotation(
    Line(points = {{0, -44}, {16, -44}}));
  connect(star.plug_p, sineVoltage.plug_n) annotation(
    Line(points = {{-78, 28}, {-62, 28}}, color = {0, 0, 255}));
  connect(sineVoltage.plug_p, pDcElec.plug_p) annotation(
    Line(points = {{-42, 28}, {-34, 28}}, color = {0, 0, 255}));
  annotation(
    Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -60}, {120, 40}})),

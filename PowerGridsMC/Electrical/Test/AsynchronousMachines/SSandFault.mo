...
//    SNom(displayUnit="V.A") = 0.1,
 //    UNom=100,
    T1=T1,
    T2=T2) annotation (Placement(transformation(origin={14,10},extent={{-20,-40},
            {0,-20}})));
  Faults.ThreePhaseFault fault(
    R=0.01,
    X=0.01,
    startTime=0.3,
    stopTime=0.6)
    annotation (Placement(transformation(origin = {0, 2}, extent = {{-30, -36}, {-20, -20}})));
equation
  connect(bus.terminal, fault.terminal) annotation(
    Line(points = {{-44, -20}, {-25, -20}, {-25, -24}}));
  connect(pgAsma2.terminal, fault.terminal) annotation(
    Line(points = {{-6, -20}, {-25, -20}, {-25, -24}}));
  annotation(
...
<p>NOTE: this model does not run on Dymola, which complains about the Fault component. This issue is not addressed yet.</p>
</body></html>"));
end SSandFault;

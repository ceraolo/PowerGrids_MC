...
equation
  ...
  annotation (
    ...
    Documentation(info = "<html><head></head><body><p>Test case for the dynamic operation of the <a href=\"modelica://PowerGridsMC.Electrical.SynchronousMachine4Windings\">SynchronousMachine4Windings</a> model. The machine, whose parameters are the same as in <a href=\"modelica://PowerGridsMC.Electrical.Test.SynchronousMachines.SynchronousMachine4Windings\">Test.SynchronousMachine4Windings</a>, except for a system frequency of 50 Hz and for a zero stator resistance, is connected to a local PQ load absorbing 495 MW of active power and 250 MVA of reactive power, which correspond to the machine power input and excitation voltage. A purely inductive line of 1 Ohm impedance, which corresponds to 0.96 p.u., connects the machine to an infinite bus, and is initially unloaded, so that all the initial voltage angles at all ports are close to zero, while the machine angle is 41.35 deg.</p>
